...
- [x] Keep top-level component folders grouped by ownership: `ui`, `footer`, `navbar`, and `shared`.
- [x] Put small reusable one-off components under `src/components/shared/` as flat `.rs` modules instead of scattering single-`mod.rs` folders.
- [x] Keep `src/components/mod.rs` as the public re-export index so route imports remain stable.
...
- [x] Batch 8 shimmer parity pass completed: `UiGradientShimmer` now renders a real canvas on WASM with idle, intro, and emphasis behavior, including stripe-wave math, intro reveal/blend, emphasis speed-up and shine boost, grain overlay, resize handling, theme color rereads, reduced-motion fallback, and deterministic CSS fallback for non-browser/server rendering. Dioxus uses `intro_key` and `emphasis_key` props instead of Svelte `bind:this` methods.
- [x] Batch 8 shimmer verification completed: `cargo fmt --check`, `cargo check`, `cargo check --no-default-features --features server`, `cargo test`, `cargo clippy --all-targets --no-default-features --features web -- -D warnings`, `dx check`, `dx build --platform web --release`, release `_headers` diff, no `.debug_*` strings in release WASM, and `git diff --check` all passed.
- [x] Batch 9 component modularization completed: flat component files moved into folder modules, `src/components/ui/mod.rs` reduced to an index, UI primitives split by control type, icons moved to `ui/icons/mod.rs`, gradient shimmer moved to `ui/gradient_shimmer/mod.rs`, and skeletons split into primitive, navigation, section, project, capability, contact, writing, article, and home modules while preserving existing `crate::components::{...}` imports.
- [x] Batch 9 verification completed: `cargo fmt --check`, `cargo check`, `cargo check --no-default-features --features server`, `cargo test`, `cargo clippy --all-targets --no-default-features --features web -- -D warnings`, `dx check`, `dx build --platform web --release`, release `_headers` diff, no `.debug_*` strings in release WASM, and `git diff --check` all passed.
- [x] Batch 9 follow-up modularization completed: `ui/icons/mod.rs` is now a small re-export index with grouped icon modules, and the gradient shimmer canvas renderer is split into config, DOM helpers, drawing, lifecycle, math, model, RNG, runtime, speed, state, stripe math, and surface/grain modules.
- [x] Batch 9 follow-up verification completed: `cargo fmt --check`, `cargo check`, `cargo check --no-default-features --features server`, `cargo test`, `cargo clippy --all-targets --no-default-features --features web -- -D warnings`, `dx check`, `dx build --platform web --release`, release `_headers` diff, no `.debug_*` strings in release WASM, and `git diff --check` all passed.
- [x] Batch 9 folder cleanup completed: component root now keeps ownership buckets for `ui`, `footer`, `navbar`, and `shared`; reusable single-component modules moved under `shared` as flat `.rs` files; route skeletons moved under the multi-file `shared/skeleton` module; header navigation extracted from `views/layout.rs` into `components/navbar`.
- [x] Batch 9 shared cleanup completed: removed one-file shared component folders so `shared` contains flat component modules plus the real multi-file `skeleton` module.
- [x] Batch 10 shimmer background completed: added one site-wide `BlueprintBackground`, mounted it behind the layout, wired mount intro, page-click emphasis, route-change emphasis, compile-time DPR cap, and reduced-motion static behavior.
- [x] Batch 10 verification completed: `cargo fmt --check`, `cargo check`, `cargo check --no-default-features --features server`, `cargo test`, `cargo clippy --all-targets --no-default-features --features web -- -D warnings`, `dx check`, `dx build --platform web --release`, release `_headers` diff, no `.debug_*` strings in release WASM, `git diff --check`, and browser/CDP visual QA passed.
- [x] Batch 10 shimmer refinement completed: removed the separate blueprint grid/vignette layers from the site background, disabled the moving CSS diagonal fallback for the background shimmer, and replaced min/target/max stripe sizing with a minimum-width fill algorithm.
- [x] Batch 10 shimmer width calibration completed: checked the local Helium Prism source and matched the background bar scale to Prism's `110px` target width while retaining the minimum-width fill behavior.
- [x] Batch 10 shimmer edge alignment fixed: removed background canvas overscan so the shimmer canvas width matches the visible viewport and the minimum-width fill algorithm no longer renders cropped half-bars at the left/right edges.
...
- [x] Review sticky header `backdrop-blur`.
  ...
  - Current decision: keep blur because the visual value is intentional, and improve the navigation design around it.
  - Proposed direction:
    - [ ] Compare screenshots with a solid/semitransparent background and no blur.
    - [x] Keep blur only if the visual value is meaningful.
  - Acceptance:
    - [x] Header remains readable and cheaper to paint, or blur is intentionally retained.
...
- [x] Decide whether the breakpoint strip should remain exposed to assistive tech.
  ...
  - Proposed direction:
    - [x] Decide whether it is content or decorative. Decision: decorative.
    - [x] If decorative, mark it `aria-hidden`.
    - [ ] If content, ensure the label and text are useful outside visual context.
  - Acceptance:
    - [x] The responsive indicator has a deliberate accessibility role.

- [x] Reconcile mobile navigation behavior with the current spec.
  ...
  - Proposed direction:
    - [x] Either update the spec to permit always-visible wrapped nav, or design a CSS-only accessible disclosure pattern.
    - [x] Avoid custom JS unless explicitly approved.
  - Acceptance:
    - [x] Mobile navigation behavior is documented and tested.
...
- [x] Review skeleton component strategy.
  ...
  - Current decision: keep skeleton exports and expand coverage for future async/server loading states; do not render fake loading over synchronous local content.
  - Proposed direction:
    - [x] Either keep skeletons as explicitly future-facing and add tests/visual references, or stop exporting them until delayed states exist.
    - [x] If kept, document where they will be used.
  - Acceptance:
    - [x] No unexplained unused exports remain.
...
## 24. Gradient Shimmer Blueprint Background Plan

Goal: integrate the ported `UiGradientShimmer` as the site-wide ambient background layer, with mount intro, route-change emphasis, and component-click emphasis. This should make the page background feel alive without forcing visitors through an experience-heavy interface.

### 24.1 User Decisions

- [x] Scope: use the shimmer as a site-wide background on every route.
- [x] Intro: play the shimmer intro when the background first mounts on initial website visit.
- [x] Interaction emphasis: trigger shimmer emphasis when any component/page content is clicked.
- [x] Route emphasis: trigger shimmer emphasis on route changes.
- [x] Performance guard: cap background canvas DPR, with the value changeable at compile time.
- [x] Bar sizing: use a minimum-width fill algorithm so bars stretch evenly until another minimum-width bar can fit.
- [x] JavaScript policy: do not add handwritten JS. Use Dioxus/Rust event handling and the existing WASM canvas renderer.

### 24.2 Product Behavior

- [x] The separate blueprint grid background is removed.
- [x] The shimmer is the only visual background layer behind content.
- [x] The shimmer is subtle during idle state.
- [x] The shimmer intro is visible on first mount but does not block rendering or navigation.
- [x] Clicking interactive content produces a short emphasis wave.
- [x] Route changes produce a short emphasis wave.
- [x] If a route-link click and route change both fire, the result feels like one extended emphasis because repeated triggers continue the existing shimmer speed-up animation instead of resetting the canvas.
- [x] Reduced-motion users receive a static background with no continuous motion.

### 24.3 Component Plan

- [x] Add `src/components/shared/blueprint_background.rs`.
- [x] Export `BlueprintBackground` from `src/components/shared/mod.rs`.
- [x] Re-export `BlueprintBackground` from `src/components/mod.rs`.
- [x] Render `BlueprintBackground` once in `src/views/layout.rs`, directly under `.blueprint-page` and before `Navbar`.
- [x] Keep `Navbar`, `main`, and `Footer` above the background through stacking context CSS.
- [x] Keep `BlueprintBackground` presentation-only with `aria-hidden="true"`.

### 24.4 Layout State Plan

- [x] In `PortfolioLayout`, create an `emphasis_key: Signal<u64>`.
- [x] Pass `emphasis_key()` to `BlueprintBackground`.
- [x] Add a page-level `onclick` handler to `.blueprint-page` that increments `emphasis_key`.
- [x] Use Dioxus event bubbling so normal links, buttons, cards, and controls trigger emphasis without wiring each component manually.
- [x] Use `use_route::<Route>()` in `PortfolioLayout` to observe current route.
- [x] Track the first observed route so initial mount does not immediately double-trigger emphasis on top of the intro.
- [x] Increment `emphasis_key` for subsequent route changes.
- [x] Do not add browser-global listeners unless Dioxus bubbling misses important interactions during testing.

### 24.5 `UiGradientShimmer` API Plan

- [x] Add an optional `max_dpr: f64` prop to `UiGradientShimmer`.
- [x] Default `max_dpr` so existing component usages keep current behavior.
- [x] Pass `max_dpr` into the WASM canvas `mount` call.
- [x] Keep the mount API simple; a canvas config struct was not needed for this single new setting.
- [x] In canvas resize handling, use capped DPR when `max_dpr` is a positive finite value.
- [x] Keep server/non-WASM rendering compatible by accepting the prop even when it is unused.
- [x] Add compile-time background constant `BACKGROUND_SHIMMER_MAX_DPR: f64 = 1.5` in `blueprint_background.rs`.
- [x] Pass that constant to `UiGradientShimmer` from `BlueprintBackground`.

### 24.6 Background Layer CSS Plan

- [x] Change `.blueprint-page` to own the base color and stacking context rather than painting the whole grid directly.
- [x] Add `.blueprint-background` as a fixed, full-viewport, pointer-events-none layer.
- [x] Add `.blueprint-background-shimmer` as the only visual background layer.
- [x] Remove `.blueprint-background-grid` from the rendered background.
- [x] Remove `.blueprint-background-vignette` from the rendered background.
- [x] Disable the moving CSS diagonal fallback for the site background shimmer.
- [x] Ensure route content and footer render above the background; navbar keeps its existing sticky `z-20` layer.
- [x] Keep the navbar readable over the animated background.
- [x] Avoid `z-index: -1` for the site background because `.blueprint-page` already paints the base background.

### 24.7 Shimmer Bar Sizing Plan

- [x] Replace min/target/max stripe sizing with minimum-width fill sizing.
- [x] Implement the mature fill rule: `count = floor(background_width / min_width).max(1)`, then `bar_width = background_width / count`.
- [x] Result: bars are never narrower than the configured minimum, stretch evenly as the viewport grows, and snap to one additional bar only when the next full minimum-width bar can fit.
- [x] Use CSS variable `--ui-shimmer-stripe-min-width` for the active minimum width.
- [x] Keep backwards-compatible reading of the old `--ui-shimmer-stripe-width-min` variable as a fallback.
- [x] Set the site background minimum width to `110px`, matching the Helium Prism target stripe width.
- [x] Remove target/max stripe sizing from the active algorithm.

### 24.8 Reduced Motion Plan

- [x] Keep existing canvas reduced-motion behavior: no active RAF loop when `prefers-reduced-motion: reduce` is enabled.
- [x] Add CSS backup that disables background shimmer animation in reduced-motion mode.
- [x] Reduce shimmer opacity in reduced-motion mode.
- [x] Confirm intro and emphasis keys do not start active motion when reduced motion is enabled.
- [x] Verify static fallback preserves the shimmer-only background without the grid.

### 24.9 Performance Plan

- [x] Keep exactly one site-wide background shimmer instance.
- [x] Cap background DPR through `BACKGROUND_SHIMMER_MAX_DPR`.
- [x] Avoid per-component shimmer backgrounds for this pass.
- [x] Avoid large blur filters on the background shimmer layer.
- [x] Remove grid and vignette layers from the site background.
- [x] Verify the release WASM still has no `.debug_*` sections.
- [x] Mobile visual QA passed with the DPR cap and current opacity, so no extra mobile-only motion reduction was needed.

### 24.10 Implementation Steps

- [x] Step 1: Add `BlueprintBackground` component with `UiGradientShimmer`.
- [x] Step 2: Add exports through `shared/mod.rs` and `components/mod.rs`.
- [x] Step 3: Mount `BlueprintBackground` in `PortfolioLayout`.
- [x] Step 4: Add layout-owned `emphasis_key` signal.
- [x] Step 5: Trigger emphasis from page-level click bubbling.
- [x] Step 6: Trigger emphasis from route changes while avoiding a duplicate first-mount trigger.
- [x] Step 7: Add `max_dpr` support to `UiGradientShimmer` and the canvas runtime.
- [x] Step 8: Remove the previous `.blueprint-page` grid background from the rendered site background.
- [x] Step 9: Tune shimmer opacity so content remains readable without grid/vignette overlays.
- [x] Step 10: Implement minimum-width shimmer bar fill sizing.
- [x] Step 11: Keep stripe alignment because the CSS-variable implementation stayed small and stable.
- [x] Step 12: Run verification and visual QA.
...
- [x] `cargo test`
- [x] `cargo clippy --all-targets --no-default-features --features web -- -D warnings`
- [x] `dx check`
- [x] `dx build --platform web --release`
- [x] Copy and diff Cloudflare `_headers` into the release public directory.
- [x] Confirm release WASM has no `.debug_*` strings.
- [x] `git diff --check`

### 24.12 Visual QA

- [x] Desktop home: shimmer sits behind all content without grid/vignette layers.
- [x] Mobile home: no content overlap, no shimmer-induced layout shift.
- [x] Projects route: background remains behind project content and route navigation preserves the background layer.
- [x] Click emphasis: clicking page content produces a visible but bounded shimmer emphasis after idle.
- [x] Route changes: route navigation triggers emphasis without visual glitching.
- [x] Writing/contact/404 risk: accepted as covered by the shared layout-level background and existing route-independent stacking model.
- [x] Reduced-motion viewport: no continuous shimmer animation.
- [x] High-DPR/mobile viewport: canvas remains sharp enough without excessive render cost.
- [x] Visual artifacts saved under `target/visual-qa/shimmer-refined-*.png`; summary reports saved under `target/visual-qa/shimmer-refined-*.json`.

### 24.13 Acceptance Criteria

- [x] The site has one integrated animated blueprint background.
- [x] Initial website visit plays the shimmer intro.
- [x] Component/page clicks trigger shimmer emphasis.
- [x] Route changes trigger shimmer emphasis.
- [x] Reduced-motion preference is respected.
- [x] Background animation does not interfere with pointer events.
- [x] Text contrast and navbar readability remain acceptable.
- [x] The DPR cap is compile-time adjustable.
- [x] Existing route behavior and content rendering remain unchanged.
